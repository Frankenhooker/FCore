    ...
    static void SendReforgePacket(Player* player, uint32 entry, uint32 lowguid = 0)
    {
        ...
        if (lowguid)
        {
            if (sObjectMgr->_itemFakeStatStore.find(lowguid) != sObjectMgr->_itemFakeStatStore.end())
                ...
                ;// remove from playerItems if exists
        }
...
public:
    ...
    {
        QueryResult result = CharacterDatabase.Query("SELECT `GUID`, `increase`, `decrease`, `stat_value` FROM `custom_reforges` WHERE `GUID` IN(SELECT `guid` FROM `item_instance` WHERE `owner_guid` = 1)");
        if (result)
        {
            do
            {
                ...
                player->_ApplyItemMods(invItem, invItem->GetSlot(), true);
                SendReforgePacket(player, invItem->GetEntry(), lowGUID);
