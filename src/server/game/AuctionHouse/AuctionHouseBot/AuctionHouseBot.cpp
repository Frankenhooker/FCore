...
void AuctionHouseBot::addNewAuctions(Player *AHBplayer, AHBConfig *config)
{
    if (!AHBSeller)
    {
        if (debug_Out) TC_LOG_INFO("auctionHouse", "AHSeller: Disabled");
        return;
    }
    ...
    if (maxItems == 0)
    {
        //if (debug_Out) TC_LOG_INFO(LOG_FILTER_GENERAL, "AHSeller: Auctions disabled");
        return;
    }
    ...
    {
        //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHSeller: Auctions above minimum");
        return;
    }

    if (auctions >= maxItems)
    {
        //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHSeller: Auctions at or above maximum");
        return;
    }
    ...
    if (debug_Out) TC_LOG_INFO("auctionHouse", "AHSeller: Adding %u Auctions", items);
    ...
    case 7:
        ...
        break;
    default:
        if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: GetAHID() - Default switch reached");
        AuctioneerGUID = 23442; //default to neutral 7
        break;
    }

    if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: Current Auctineer GUID is %u", AuctioneerGUID);
...
*/
    ...
    if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: %u items", items);

    // only insert a few at a time, so as not to peg the processor
    for (uint32 cnt = 1; cnt <= items; cnt++)
    {
        if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: %u count", cnt);
        uint32 itemID = 0;
        uint32 itemColor = 99;
        ...
        while (itemID == 0 && loopbreaker <= 50)
        {
            ...
            default:
                {
                       if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: itemID Switch - Default Reached");
                    break;
                }
            }
            ...
            if (itemID == 0)
            {
                if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: Item::CreateItem() - ItemID is 0");
                continue;
            }
            ...
            if (prototype == NULL)
            {
                if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: Huh?!?! prototype == NULL");
                continue;
            }
            ...
            if (item == NULL)
            {
                if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHSeller: Item::CreateItem() returned NULL");
                break;
            }
            ...
            else
            {
                ...
                if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHBuyer: Quality %u not Supported", prototype->Quality);
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    if (!AHBBuyer)
    {
        if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHBuyer: Disabled");
        return;
    }
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        {
            //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHBuyer: I have no items to bid on.");
        ...
        if (!pItem)
        {
            if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHBuyer: Item %u doesn't exist, perhaps bought already?", auction->itemGUIDLow);
            continue;
        }
        ...
        if (debug_Out)
        {
            TC_LOG_INFO("auctionHouse", "-------------------------------------------------");
            TC_LOG_INFO("auctionHouse", "AHBuyer: Info for Auction #%u:", auction->Id);
            TC_LOG_INFO("auctionHouse", "AHBuyer: AuctionHouse: %u", auction->GetHouseId());
            TC_LOG_INFO("auctionHouse", "AHBuyer: Auctioneer: %u", auction->auctioneer);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Owner: %u", auction->owner);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Bidder: %u", auction->bidder);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Starting Bid: %u", auction->startbid);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Current Bid: %u", currentprice);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Buyout: %u", auction->buyout);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Deposit: %u", auction->deposit);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Expire Time: %u", uint32(auction->expire_time));
            TC_LOG_INFO("auctionHouse", "AHBuyer: Bid Rate: %f", bidrate);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Bid Max: %Lf", bidMax);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Bid Value: %Lf", bidvalue);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Bid Price: %u", bidprice);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Item GUID: %u", auction->itemGUIDLow);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Item Template: %u", auction->itemEntry);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Item Info:");
            TC_LOG_INFO("auctionHouse", "AHBuyer: Item ID: %u", prototype->ItemId);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Buy Price: %u", prototype->BuyPrice);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Sell Price: %u", prototype->SellPrice);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Bonding: %u", prototype->Bonding);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Quality: %u", prototype->Quality);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Item Level: %u", prototype->ItemLevel);
            TC_LOG_INFO("auctionHouse", "AHBuyer: Ammo Type: %u", prototype->AmmoType);
            TC_LOG_INFO("auctionHouse", "-------------------------------------------------");
        }
...
void AuctionHouseBot::Update()
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
    {
        ...
        {
            //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_a));
            //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHBuyer: Bidding on Alliance Auctions");
        ...
        {
            //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_h));
            //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHBuyer: Bidding on Horde Auctions");
    ...
    {
        //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_n));
        //if (debug_Out) TC_LOG_ERROR(LOG_FILTER_GENERAL, "AHBuyer: Bidding on Neutral Auctions");
...
void AuctionHouseBot::Initialize()
{
    ...
    if (AHBSeller)
    {
        ...
        {
            if (debug_Out) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: \"%s\" failed", npcQuery);
        }
        ...
        {
            if (debug_Out) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: \"%s\" failed", lootQuery);
        }
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (PTR/Beta/Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Permanent Enchant Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Conjured Consumable)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Gem)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Money)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (MoneyLoot)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Lootable Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Quest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Has a Duration)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (BOP or BQI and Required Level is less than Item Level)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Warrior Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Paladin Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Hunter Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Rogue Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Priest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (DK Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Shaman Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Mage Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Warlock Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Druid Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Item Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
           ...
           // Disable Trade Goods below level X
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Trade Good %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
           ...
           // Disable Trade Goods above level X
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Trade Good %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            // {
            //    if (debug_Out_Filters) TC_LOG_ERROR("auctionHouse", "AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", itr->second.ItemId, itr->second.RequiredSkillRank);
            //    continue;
            // }
...
void AuctionHouseBot::IncrementItemCounts(AuctionEntry* ah)
{
    ...
    if (!pItem)
    {
        if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHBot: Item %u doesn't exist, perhaps bought already?", ah->itemGUIDLow);
        return;
    }
    ...
    if (!u_entry)
    {
        if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHBot: %u returned as House Faction. Neutral", ah->GetHouseFaction());
        config = &NeutralConfig;
    }
    else if (u_entry->ourMask & FACTION_MASK_ALLIANCE)
    {
        if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHBot: %u returned as House Faction. Alliance", ah->GetHouseFaction());
        config = &AllianceConfig;
    }
    else if (u_entry->ourMask & FACTION_MASK_HORDE)
    {
        if (debug_Out) TC_LOG_ERROR("auctionHouse", "AHBot: %u returned as House Faction. Horde", ah->GetHouseFaction());
        config = &HordeConfig;
    }
    else
    {
...
{
    if (debug_Out)
        TC_LOG_ERROR("auctionHouse", "Start Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
    if (AHBSeller)
    {
        ...
        if (debug_Out)
        {
            TC_LOG_ERROR("auctionHouse", "minItems                = %u", config->GetMinItems());
            TC_LOG_ERROR("auctionHouse", "maxItems                = %u", config->GetMaxItems());
            TC_LOG_ERROR("auctionHouse", "percentGreyTradeGoods   = %u", config->GetPercentages(AHB_GREY_TG));
            TC_LOG_ERROR("auctionHouse", "percentWhiteTradeGoods  = %u", config->GetPercentages(AHB_WHITE_TG));
            TC_LOG_ERROR("auctionHouse", "percentGreenTradeGoods  = %u", config->GetPercentages(AHB_GREEN_TG));
            TC_LOG_ERROR("auctionHouse", "percentBlueTradeGoods   = %u", config->GetPercentages(AHB_BLUE_TG));
            TC_LOG_ERROR("auctionHouse", "percentPurpleTradeGoods = %u", config->GetPercentages(AHB_PURPLE_TG));
            TC_LOG_ERROR("auctionHouse", "percentOrangeTradeGoods = %u", config->GetPercentages(AHB_ORANGE_TG));
            TC_LOG_ERROR("auctionHouse", "percentYellowTradeGoods = %u", config->GetPercentages(AHB_YELLOW_TG));
            TC_LOG_ERROR("auctionHouse", "percentGreyItems        = %u", config->GetPercentages(AHB_GREY_I));
            TC_LOG_ERROR("auctionHouse", "percentWhiteItems       = %u", config->GetPercentages(AHB_WHITE_I));
            TC_LOG_ERROR("auctionHouse", "percentGreenItems       = %u", config->GetPercentages(AHB_GREEN_I));
            TC_LOG_ERROR("auctionHouse", "percentBlueItems        = %u", config->GetPercentages(AHB_BLUE_I));
            TC_LOG_ERROR("auctionHouse", "percentPurpleItems      = %u", config->GetPercentages(AHB_PURPLE_I));
            TC_LOG_ERROR("auctionHouse", "percentOrangeItems      = %u", config->GetPercentages(AHB_ORANGE_I));
            TC_LOG_ERROR("auctionHouse", "percentYellowItems      = %u", config->GetPercentages(AHB_YELLOW_I));
            TC_LOG_ERROR("auctionHouse", "minPriceGrey            = %u", config->GetMinPrice(AHB_GREY));
            TC_LOG_ERROR("auctionHouse", "maxPriceGrey            = %u", config->GetMaxPrice(AHB_GREY));
            TC_LOG_ERROR("auctionHouse", "minPriceWhite           = %u", config->GetMinPrice(AHB_WHITE));
            TC_LOG_ERROR("auctionHouse", "maxPriceWhite           = %u", config->GetMaxPrice(AHB_WHITE));
            TC_LOG_ERROR("auctionHouse", "minPriceGreen           = %u", config->GetMinPrice(AHB_GREEN));
            TC_LOG_ERROR("auctionHouse", "maxPriceGreen           = %u", config->GetMaxPrice(AHB_GREEN));
            TC_LOG_ERROR("auctionHouse", "minPriceBlue            = %u", config->GetMinPrice(AHB_BLUE));
            TC_LOG_ERROR("auctionHouse", "maxPriceBlue            = %u", config->GetMaxPrice(AHB_BLUE));
            TC_LOG_ERROR("auctionHouse", "minPricePurple          = %u", config->GetMinPrice(AHB_PURPLE));
            TC_LOG_ERROR("auctionHouse", "maxPricePurple          = %u", config->GetMaxPrice(AHB_PURPLE));
            TC_LOG_ERROR("auctionHouse", "minPriceOrange          = %u", config->GetMinPrice(AHB_ORANGE));
            TC_LOG_ERROR("auctionHouse", "maxPriceOrange          = %u", config->GetMaxPrice(AHB_ORANGE));
            TC_LOG_ERROR("auctionHouse", "minPriceYellow          = %u", config->GetMinPrice(AHB_YELLOW));
            TC_LOG_ERROR("auctionHouse", "maxPriceYellow          = %u", config->GetMaxPrice(AHB_YELLOW));
            TC_LOG_ERROR("auctionHouse", "minBidPriceGrey         = %u", config->GetMinBidPrice(AHB_GREY));
            TC_LOG_ERROR("auctionHouse", "maxBidPriceGrey         = %u", config->GetMaxBidPrice(AHB_GREY));
            TC_LOG_ERROR("auctionHouse", "minBidPriceWhite        = %u", config->GetMinBidPrice(AHB_WHITE));
            TC_LOG_ERROR("auctionHouse", "maxBidPriceWhite        = %u", config->GetMaxBidPrice(AHB_WHITE));
            TC_LOG_ERROR("auctionHouse", "minBidPriceGreen        = %u", config->GetMinBidPrice(AHB_GREEN));
            TC_LOG_ERROR("auctionHouse", "maxBidPriceGreen        = %u", config->GetMaxBidPrice(AHB_GREEN));
            TC_LOG_ERROR("auctionHouse", "minBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            TC_LOG_ERROR("auctionHouse", "maxBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            TC_LOG_ERROR("auctionHouse", "minBidPricePurple       = %u", config->GetMinBidPrice(AHB_PURPLE));
            TC_LOG_ERROR("auctionHouse", "maxBidPricePurple       = %u", config->GetMaxBidPrice(AHB_PURPLE));
            TC_LOG_ERROR("auctionHouse", "minBidPriceOrange       = %u", config->GetMinBidPrice(AHB_ORANGE));
            TC_LOG_ERROR("auctionHouse", "maxBidPriceOrange       = %u", config->GetMaxBidPrice(AHB_ORANGE));
            TC_LOG_ERROR("auctionHouse", "minBidPriceYellow       = %u", config->GetMinBidPrice(AHB_YELLOW));
            TC_LOG_ERROR("auctionHouse", "maxBidPriceYellow       = %u", config->GetMaxBidPrice(AHB_YELLOW));
            TC_LOG_ERROR("auctionHouse", "maxStackGrey            = %u", config->GetMaxStack(AHB_GREY));
            TC_LOG_ERROR("auctionHouse", "maxStackWhite           = %u", config->GetMaxStack(AHB_WHITE));
            TC_LOG_ERROR("auctionHouse", "maxStackGreen           = %u", config->GetMaxStack(AHB_GREEN));
            TC_LOG_ERROR("auctionHouse", "maxStackBlue            = %u", config->GetMaxStack(AHB_BLUE));
            TC_LOG_ERROR("auctionHouse", "maxStackPurple          = %u", config->GetMaxStack(AHB_PURPLE));
            TC_LOG_ERROR("auctionHouse", "maxStackOrange          = %u", config->GetMaxStack(AHB_ORANGE));
            TC_LOG_ERROR("auctionHouse", "maxStackYellow          = %u", config->GetMaxStack(AHB_YELLOW));
        }
        //AuctionHouseEntry const* ahEntry =  sAuctionMgr->GetAuctionHouseEntry(config->GetAHFID());
        AuctionHouseObject* auctionHouse =  sAuctionMgr->GetAuctionsMap(config->GetAHFID());
        ...
        if (debug_Out)
        {
            TC_LOG_ERROR("auctionHouse", "Current Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
            TC_LOG_ERROR("auctionHouse", "Grey Trade Goods\t%u\tGrey Items\t%u", config->GetItemCounts(AHB_GREY_TG), config->GetItemCounts(AHB_GREY_I));
            TC_LOG_ERROR("auctionHouse", "White Trade Goods\t%u\tWhite Items\t%u", config->GetItemCounts(AHB_WHITE_TG), config->GetItemCounts(AHB_WHITE_I));
            TC_LOG_ERROR("auctionHouse", "Green Trade Goods\t%u\tGreen Items\t%u", config->GetItemCounts(AHB_GREEN_TG), config->GetItemCounts(AHB_GREEN_I));
            TC_LOG_ERROR("auctionHouse", "Blue Trade Goods\t%u\tBlue Items\t%u", config->GetItemCounts(AHB_BLUE_TG), config->GetItemCounts(AHB_BLUE_I));
            TC_LOG_ERROR("auctionHouse", "Purple Trade Goods\t%u\tPurple Items\t%u", config->GetItemCounts(AHB_PURPLE_TG), config->GetItemCounts(AHB_PURPLE_I));
            TC_LOG_ERROR("auctionHouse", "Orange Trade Goods\t%u\tOrange Items\t%u", config->GetItemCounts(AHB_ORANGE_TG), config->GetItemCounts(AHB_ORANGE_I));
            TC_LOG_ERROR("auctionHouse", "Yellow Trade Goods\t%u\tYellow Items\t%u", config->GetItemCounts(AHB_YELLOW_TG), config->GetItemCounts(AHB_YELLOW_I));
        }
    }
    if (AHBBuyer)
    {
        ...
        if (debug_Out)
        {
            TC_LOG_ERROR("auctionHouse", "buyerPriceGrey          = %u", config->GetBuyerPrice(AHB_GREY));
            TC_LOG_ERROR("auctionHouse", "buyerPriceWhite         = %u", config->GetBuyerPrice(AHB_WHITE));
            TC_LOG_ERROR("auctionHouse", "buyerPriceGreen         = %u", config->GetBuyerPrice(AHB_GREEN));
            TC_LOG_ERROR("auctionHouse", "buyerPriceBlue          = %u", config->GetBuyerPrice(AHB_BLUE));
            TC_LOG_ERROR("auctionHouse", "buyerPricePurple        = %u", config->GetBuyerPrice(AHB_PURPLE));
            TC_LOG_ERROR("auctionHouse", "buyerPriceOrange        = %u", config->GetBuyerPrice(AHB_ORANGE));
            TC_LOG_ERROR("auctionHouse", "buyerPriceYellow        = %u", config->GetBuyerPrice(AHB_YELLOW));
            TC_LOG_ERROR("auctionHouse", "buyerBiddingInterval    = %u", config->GetBiddingInterval());
            TC_LOG_ERROR("auctionHouse", "buyerBidsPerInterval    = %u", config->GetBidsPerInterval());
        }
    }
    if (debug_Out) TC_LOG_ERROR("auctionHouse", "End Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
}
